"""
USAA Federal Savings Bank statement parser.
...
Credit card layout:
  Date | Description | Amount (positive = charge)

USAA often has a "USAA" watermark/header and transaction
sections labeled "Transactions" or "Account Activity".
"""
...
_CC_SIGNALS = [
    "credit limit", "minimum payment due", "payment due date",
    "new balance", "purchase apr", "cash advance",
]
...
class USAAParser(BaseParser):
    ...
    def extract(self) -> ParsedStatement:
        ...
        account = self.build_account(text, bank_name="USAA")
        if is_cc:
        ...
        stmt = ParsedStatement(
            ...
            parser_used="usaa_cc" if is_cc else "usaa",
    ...
    def _parse_table(self, table: list[list[str]], is_cc: bool) -> list[Transaction]:
        ...
        for row in table:
            ...
            try:
                ...
                self.warn(f"USAA table row error: {e}")
        return txns
